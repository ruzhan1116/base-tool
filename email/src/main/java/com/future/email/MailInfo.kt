...
class MailInfo {
    ...
    val properties: Properties
        get() {
            val p = Properties()
            p.setProperty(MAIL_SMTP_HOST, this.mailServerHost)
            p.setProperty(MAIL_SMTP_PORT, this.mailServerPort)
            p.setProperty(MAIL_SMTP_AUTH, if (isValidate) TRUE else FALSE)
            return p
        }
}
